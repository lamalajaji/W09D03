...
const List = () => {
  ...
  const remove = async (id, token) => {
    try {
      const res = await axios.put(
        `${process.env.REACT_APP_BASE_URL}/todo/${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      <div>
        {!state.token ? (
          ...
          <div>
            ...
            />
            <button onClick={() => addTask(state.token)}> Add</button>
          </div>
        )}
          ...
          <ul>
            {state.list.map((task) => (
              <div className="item" key={task._id}>
                ...
                <button onClick={() => edit(task._id, state.token)}>
                  Edit
                </button>
                <button onClick={() => remove(task._id, state.token)}>
                  Remove
                </button>
              </div>
            ))}
